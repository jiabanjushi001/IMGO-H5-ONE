const ImgoRolePanel = {
  ...
  data() {
    return {
      roles: [], permissions: [], activeRoleID: 0, loading: false, saving: false,
      form: { role_id: 0, name: '', remark: '', status: 1, permissions: [] }
    }
  },
  mounted() { this.load() },
  methods: {
    async load(preferredID) {
      ...
      try {
        ...
        const selected = this.roles.find(role => Number(role.role_id) === Number(preferredID || this.activeRoleID))
    ...
    create() {
      this.activeRoleID = 0
      this.form = { role_id: 0, name: '', remark: '', status: 1, permissions: [] }
    },
    select(role) {
  ...
  render(h) {
    const roleCards = this.roles.map(role => h('button', {
      ...
      h('span', { class: ['imgo-role-card-status', Number(role.status) === 1 ? 'is-enabled' : 'is-disabled'] }, Number(role.status) === 1 ? '已启用' : '已禁用')
    ]))
    ...
    return h('section', { class: 'imgo-role-page' }, [
      h('header', { class: 'imgo-role-heading' }, [
        h('div', [h('h1', '角色权限'), h('p', '为后台人员分配一个角色，并同时限制菜单和接口权限。')]),
      ...
      h('div', { class: 'imgo-role-layout', directives: [{ name: 'loading', value: this.loading }] }, [
        ...
        h('main', { class: 'imgo-role-editor' }, [
          h('div', { class: 'imgo-role-editor-title' }, this.form.role_id ? '编辑角色' : '新增角色'),
          h('el-form', { props: { labelWidth: '92px' } }, [
            h('el-form-item', { props: { label: '角色名称', required: true } }, [h('el-input', { props: { value: this.form.name, maxlength: 64, showWordLimit: true, placeholder: '例如：客服、群聊管理员' }, on: { input: value => { this.form.name = value } } })]),
            h('el-form-item', { props: { label: '角色状态' } }, [h('el-switch', { props: { value: this.form.status, activeValue: 1, inactiveValue: 0, activeText: '启用', inactiveText: '禁用' }, on: { input: value => { this.form.status = Number(value) } } })]),
            h('el-form-item', { props: { label: '角色备注' } }, [h('el-input', { props: { value: this.form.remark, type: 'textarea', rows: 3, maxlength: 255, showWordLimit: true, placeholder: '说明该角色的使用范围' }, on: { input: value => { this.form.remark = value } } })]),
            h('el-form-item', { props: { label: '菜单权限' } }, [h('el-checkbox-group', { class: 'imgo-role-permissions', props: { value: this.form.permissions }, on: { input: value => { this.form.permissions = value } } }, permissionBoxes)])
          ]),
          h('div', { class: 'imgo-role-actions' }, [
