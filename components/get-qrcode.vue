...
export default {
	...
	mounted() {
		if (!window.isSecureContext || !navigator.mediaDevices?.getUserMedia) {
			this.isUse = false
			this.$emit('error', new Error('当前页面无法使用实时摄像头'))
			return
		}
		...
		openScan() {
			...
			navigator.mediaDevices
				...
				.then(stream => {
					...
					this.video.play().catch(err => this.$emit('error', err))
					this.tick()

					this.track = stream.getVideoTracks()[0]
					setTimeout(() => {
						this.isUseTorch = !!this.track?.getCapabilities?.().torch
					}, 500)
				})
				.catch(err => {
					this.isUse = false
